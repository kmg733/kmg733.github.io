...
그림의 마지막 분기가 인프라 제약 단계입니다. 여기서 SSE와 롱 폴링이 갈리는 조건은 앞 SSE 절에서 짚은 두 가지입니다. HTTP/1.1의 도메인당 연결 6개 제한과 프록시 버퍼링을 통제할 수 있으면 SSE로 가고, 그러지 못하는 환경에서만 롱 폴링으로 물러섭니다. 롱 폴링이 트리 맨 끝에 놓였다고 더 나은 방식은 아닙니다. SSE를 쓸 수 없을 때 같은 서버 푸시 효과를 내는 대체재이고, SSE라면 브라우저가 공짜로 주는 자동 재연결과 `Last-Event-ID` 복구를 직접 구현해야 합니다.
