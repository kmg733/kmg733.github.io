...
회사 프로젝트에서 다국어 지원 인프라를 구축할 기회가 생겼습니다.
프로젝트 환경은 Spring Boot + Thymeleaf 기반의 MPA(Multi-Page Application)였고,
서버 렌더링과 클라이언트 JavaScript 양쪽에서 번역 메시지를 사용할 수 있어야 했습니다.

기존 레거시 프로젝트에서는 `jquery.i18n.properties.js`를 사용하고 있었는데,
페이지마다 `.properties` 파일을 별도로 fetch하는 구조, 서버 내부 메시지까지 클라이언트에 노출되는 보안 문제,
그리고 jQuery에 대한 강한 의존 등 여러 한계가 있었습니다.

이러한 문제를 해결하면서도 네트워크 요청을 최소화할 방법을 고민한 결과,
서버 사이드의 <Term id="message-source">MessageSource</Term>와 클라이언트 사이드의 <Term id="i18next">i18next</Term>를 결합하고
localStorage 기반 <Term id="swr">SWR 캐싱</Term>으로 캐시 히트 시 네트워크 요청 0회를 달성하는 하이브리드 다국어 아키텍처를 설계하게 되었습니다.
이 글에서는 그 아키텍처의 전체 구조와 구현을 다룹니다.

---

## 기존 방식의 한계

레거시 프로젝트에서 사용하던 `jquery.i18n.properties.js` 플러그인의 문제를 구체적으로 살펴보겠습니다.
...
<div className="info-box">
  <strong>SWR 캐싱 패턴이 처음이라면</strong><br/>
  이 섹션은 Spring Boot 아키텍처 관점에서 SWR 캐싱을 다룹니다.
  SWR의 기본 개념과 동작 원리를 일상 비유부터 단계적으로 이해하고 싶다면
  <a href="/blog/localstorage-swr-caching-pattern">localStorage SWR 캐싱 패턴</a> 글을 먼저 읽어보는 것을 권장합니다.
</div>
...
서버 데이터가 변경되는 빈도(배포 시점)보다 사용자가 방문하는 빈도가 훨씬 높기 때문입니다.
즉, 대부분의 페이지 로딩에서 네트워크 요청이 0회입니다.
...
- [localStorage SWR 캐싱 패턴](/blog/localstorage-swr-caching-pattern) — SWR 캐싱의 기본 개념, 3가지 캐시 전략 비교, 해시 기반 무효화 원리를 비개발자도 이해할 수 있도록 설명합니다.
