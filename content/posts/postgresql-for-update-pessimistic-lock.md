...
여러 사용자가 웹에서 같은 작업 항목에 대해 **강제삭제(force delete)** 와 **복구(recover)** 를 거의 동시에 요청할 수 있었습니다.
요청은 모두 웹에서 들어오지만, **처리 주체가 갈렸습니다** — 복구는 **웹 서버**가, 강제삭제는 **에이전트 서버**가 맡았습니다.
그래서 같은 항목에 두 요청이 겹치면 서로 다른 두 서버(웹·에이전트)가 같은 행을 동시에 건드리게 되고, 각 처리는 "현재 상태가 삭제 가능한가"를 확인한 뒤 진행하므로 확인과 처리 사이(TOCTOU)에 경합이 발생했습니다.

```
에이전트 서버 (강제삭제 처리):  상태 조회 → '삭제실패' 확인 → 물리 삭제
웹 서버 (복구 처리):                       상태 조회 → '삭제실패' 확인 → 상태 복구
                              └ 두 흐름이 겹치면: 복구된 대상을 삭제하거나, 없는 행을 삭제
```

처리 주체가 서로 다른 프로세스라 애플리케이션 레벨 락으로는 막을 수 없었고, 앞서 봤듯이 일반 조회·변경만으로는 예외 없이 경합이 통과되므로, 검증에 쓰는 조회 구간부터 DB 행을 직접 잠가야 했습니다.
...
- **서버 경계를 넘는 직렬화** — 잠금이 DB에 있으므로, 어느 서버가 처리하든 같은 행에 대한 처리는 하나씩 직렬화됩니다.

---

## 조건부 UPDATE로는 안 될까 — WHERE 필터와의 경계

여기까지 보면 이런 의문이 듭니다.
"굳이 `SELECT ... FOR UPDATE`로 잠그지 말고, `UPDATE` 문에 조건을 붙이면 되지 않나?"
맞습니다. **단순한 경우엔 조건부 UPDATE가 오히려 더 낫습니다.**

### 조건부 UPDATE로 충분한 경우

"현재 값 확인 → 변경"이 **SQL 한 문장으로 표현되면** FOR UPDATE 없이도 안전합니다.

```sql
-- status가 아직 READY일 때만 성공
UPDATE job_t SET status = 'RUNNING'
WHERE id = #{id} AND status = 'READY';
-- 영향 행 수가 0이면 그 사이 남이 바꾼 것 → 경합 감지

-- 재고: 남은 수량이 있을 때만 차감 (초과 판매 방지)
UPDATE item_t SET stock = stock - 1
WHERE id = #{id} AND stock >= 1;
```

READ COMMITTED에서 UPDATE는 잠긴 행을 만나면 대기했다가, 잠금이 풀린 뒤 `WHERE` 조건을 **최신 행 버전으로 다시 평가**합니다.
앞 트랜잭션이 값을 바꿨으면 조건에 안 맞아 0건 처리되므로, 이 자체로 경합이 걸러집니다.
왕복 한 번에 명시적 잠금 관리도 없어 더 간결합니다.

> 앞서 Spring 예시로 든 `startJob`(READY→RUNNING)도 사실 이 단순 케이스에 해당합니다. 이 글에서는 `FOR UPDATE`의 동작을 보여주려 잠금 방식으로 구현했지만, 단일 상태 전이라면 위 조건부 UPDATE가 더 간결한 선택입니다.

### FOR UPDATE가 필요한 경우

반대로 "확인 → 변경"이 한 문장으로 안 떨어지면 조회 시점 잠금이 필요합니다.

| 상황 | 조건부 UPDATE로 안 되는 이유 |
|------|------------------------------|
| **검증이 앱 로직** | 여러 테이블 집계·권한·비즈니스 규칙 등 `WHERE` 술어로 표현 불가 |
| **읽은 값으로 여러 번 쓰기** | 이체처럼 잔액 읽고 출금+입금+원장 기록. 그 사이 값이 고정돼야 함 |
| **값 자체가 필요** | "블라인드 쓰기"인 조건부 UPDATE와 달리, 값을 읽어 계산·응답·분기해야 할 때 |
| **경합 종류 구분** | 조건부 문장은 "잠김"과 "대상 아님"을 모두 0건으로 뭉뚱그림 |

### 이번 강제삭제 사례에서 조건부 문장으로 안 됐던 이유

앞의 강제삭제 역시 처음에는 조건부 `DELETE`(`... WHERE delete_status IN (...)`) 한 문장으로 끝내려 했습니다. 하지만 다음 세 가지 때문에 조건부 문장만으로는 부족했습니다.

1. **"처리 중"과 "삭제 불가"를 구분해야 했다** — 조건부 DELETE는 실패 원인이 무엇이든(다른 서버가 잠금 보유, 상태 부적합, 대상 없음) 똑같이 **0건**을 반환해 셋을 구분할 수 없습니다. 그런데 "다른 작업이 처리 중입니다(잠시 후 재시도)"와 "삭제 가능한 상태가 아닙니다"는 **다르게 안내**해야 했습니다.

   그럼 `FOR UPDATE`는 어떻게 구분할까요? 잠금 시도를 **예외**와 **반환값**이라는 서로 다른 경로로 갈라주기 때문입니다.

   | 상황 | `FOR UPDATE NOWAIT` 결과 | 안내 |
   |------|--------------------------|------|
   | 다른 서버가 그 행을 잠금 (처리 중) | **예외** (55P03) | "다른 작업이 처리 중입니다" |
   | 잠기지 않은 행 → 상태 검증 | status **반환** | 부적합이면 "삭제 가능한 상태가 아닙니다", 적합이면 삭제 진행 |
   | 대상 행이 없음 | **`null`** 반환 | 필요 시 "대상 없음"으로 별도 처리 |

   즉 "처리 중(잠김)"은 **예외**로 튀고, "삭제 불가(상태 부적합)"는 **반환값**으로 오므로 둘을 갈라 안내할 수 있습니다. 잠긴 행은 애초에 그 행이 **존재**할 때만 생기고, 행이 없으면 예외 대신 `null`이 오는 것도 이 구분을 뒷받침합니다.
2. **즉시 실패가 필요했는데 `NOWAIT`을 붙일 수 없었다** — 웹 요청이라 잠긴 행을 만나면 대기 대신 바로 되돌려야 했지만, `NOWAIT`은 `SELECT ... FOR UPDATE`(와 `LOCK TABLE`)에만 붙습니다. `UPDATE`/`DELETE` 문에는 `NOWAIT`이 없어 잠긴 행에서 무한 대기하게 됩니다. 즉시 실패로 구분하려면 `SELECT ... FOR UPDATE NOWAIT`이 필요했습니다.
3. **확인 후 삭제가 여러 단계였다** — 상태 검증(앱의 집합 판정) 뒤 물리 삭제가 한 문장이 아니라 여러 작업으로 이어져, 잠금을 유지한 채 진행해야 했습니다.

결국 "잠겨 있으면 즉시 실패(NOWAIT), 아니면 잠금을 쥔 채 검증·삭제"라는 흐름이 필요했고, 이는 조건부 UPDATE 한 문장으로는 표현할 수 없었습니다.
...
| **조건부 UPDATE와 경계** | "확인→변경"이 단일 문장 컬럼 조건이면 `WHERE` 필터가 더 간결. 앱 판단·다단계 쓰기·경합 종류 구분(NOWAIT)이 필요하면 FOR UPDATE |
...
그러면 어느 서버가 처리하든 같은 행에 대한 처리 순서가 보장됩니다.
