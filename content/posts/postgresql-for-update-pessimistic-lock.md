...
1. **"처리 중"과 "삭제 불가"를 구분해야 했다** — 조건부 DELETE는 실패 원인이 무엇이든(다른 서버가 잠금 보유, 상태 부적합, 대상 없음) 똑같이 **0건**을 반환해 셋을 구분할 수 없습니다. 그런데 "다른 작업이 처리 중입니다(잠시 후 재시도)"와 "삭제 가능한 상태가 아닙니다"는 **다르게 안내**해야 했습니다.
   ...
   반면 `FOR UPDATE`는 이를 **예외와 반환값이라는 서로 다른 경로**로 갈라줍니다. 다른 서버가 그 행을 잠근 "처리 중"이면 `NOWAIT`이 예외(55P03)로 튀고, 잠기지 않은 행이면 status를 반환해 앱에서 상태를 검증합니다. 즉 "처리 중(잠김)"은 예외로, "삭제 불가(상태 부적합)"는 반환값으로 오므로 둘을 갈라 안내할 수 있습니다. 잠긴 행은 그 행이 **존재**할 때만 생기고, 행이 없으면 예외 대신 `null`이 오는 것도 이 구분을 뒷받침합니다.
