...
앞의 강제삭제도 처음엔 `DELETE ... WHERE delete_status IN (...)` 한 방으로 처리하려 했지만 막혔습니다. 세 가지 때문이었습니다.

1. **"처리 중"과 "삭제 불가"를 구분해야 했다** — 조건부 DELETE는 다른 서버가 그 행을 잠근 경우든 상태가 부적합한 경우든 똑같이 **0건**을 반환합니다. 그런데 전자는 "다른 작업이 처리 중입니다(잠시 후 재시도)", 후자는 "삭제 가능한 상태가 아닙니다"로 **다르게 안내**해야 했습니다.
