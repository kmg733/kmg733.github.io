...
사내에서 운영하는 제품의 관리 콘솔 이야기입니다. 화면 이름과 구조는 일반화해서 적었습니다. 한 대시보드 안에 실시간 화면이 둘 있었습니다. 하나는 데이터가 처리 파이프라인을 통과하는 모습을 초 단위로 보여 주는 **처리 파이프라인 화면**(신규), 다른 하나는 통계 위젯 열한 개를 모아 놓은 **요약 대시보드 화면**(개선)입니다.

<figure>
  <div className="figure-content">
    <div className="image-frame">
      <img className="theme-light" src="/images/posts/sse-production-implementation/dashboard-split-light.png" alt="처리 파이프라인 화면은 SSE, 요약 대시보드 화면은 폴링으로 구성한 구조" />
      <img className="theme-dark" src="/images/posts/sse-production-implementation/dashboard-split-dark.png" alt="처리 파이프라인 화면은 SSE, 요약 대시보드 화면은 폴링으로 구성한 구조" />
    </div>
  </div>
  <figcaption>그림 2. 같은 대시보드 안의 두 화면이 서로 다른 방식을 쓴다</figcaption>
</figure>

### 처리 파이프라인 화면에 SSE를 쓴 이유

먼저 이 화면이 무엇인지 그려 두겠습니다. 여러 처리 작업이 동시에 흘러가는 모습을 실시간으로 보여 주는 화면입니다. 작업 하나가 가로 레인 하나가 되고, 각 레인의 데이터가 수집 → 분석 → 검출 → 저장이라는 네 단계를 지나갑니다. 처리량이 늘어난 만큼 단계 사이를 작은 점들이 왼쪽에서 오른쪽으로 흘러가고, 레인마다 진행률 막대와 경과 시간, 초당 처리 건수가 붙습니다. 화면 아래에는 전체 작업 수와 누적 처리 건수 같은 요약이 놓입니다. 클라이언트는 매 갱신마다 직전 스냅샷과의 차이를 계산해 그 차이만큼 점을 흘려보냅니다. 이 "차이로 점을 만든다"는 구조가 뒤의 순서 문제와 이어집니다.

서버에서 조회하는 데이터는 이 파이프라인 상태 하나뿐이고, 화면 전체가 그 하나를 바라봅니다. SSE로 간 이유는 세 가지였습니다.
...
**게다가 그 주기를 관리자가 런타임에 바꿉니다.** 폴링에서는 이게 공짜입니다. 클라이언트가 설정값을 읽어 위젯별 타이머를 걸고, 설정이 바뀌면 타이머를 다시 겁니다. SSE로 만들면 이 주기가 서버 상태가 됩니다. 위젯별 스케줄 열한 개를 서버가 들고 돌리고, 값이 바뀔 때마다 런타임에 다시 등록해야 합니다. 그 비용은 파이프라인 쪽에서 이미 치러 봤습니다. 동적 주기가 하나뿐인데도 `@Scheduled`로는 표현할 수 없어 `Trigger`와 전용 스케줄러를 직접 만들어야 했습니다.
...
이렇게 처리 파이프라인 화면은 SSE로 갔습니다. 화면을 띄우는 것 자체는 쉬웠습니다. 어려운 건 그다음이었습니다.
